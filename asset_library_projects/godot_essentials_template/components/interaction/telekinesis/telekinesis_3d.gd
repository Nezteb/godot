@icon("res://components/interaction/telekinesis/telekinesis.svg")
...
@export_range(0.1, 100.0, 0.01) var throwable_interactor_distance = 5.0:
	set(value):
		...
		throwable_interactor_distance = value
...
var active_throwables: Array[Throwable3D] = []


func _unhandled_input(_event: InputEvent):
	if InputWizard.action_just_pressed_and_exists(throw_input_action):
		var body := active_throwables.pop_front() as Throwable3D
		
		if body_can_be_lifted(body):
			pull_body(body)
			
	if pull_individual_ability and InputWizard.action_just_pressed_and_exists(pull_input_action):
		if throwable_interactor and throwable_interactor.is_colliding():
			var body = throwable_interactor.get_collider() as Throwable3D
			if body_can_be_lifted(body):
				pull_body(body)
	
	if pull_area_ability and InputWizard.action_just_pressed_and_exists(pull_area_input_action):
		for body: Throwable3D in get_near_throwables():
			if body_can_be_lifted(body):
				pull_body(body)
				
	if push_wave_ability and InputWizard.action_just_pressed_and_exists(push_wave_input_action):
		push_wave()
...
func _physics_process(delta):
	for body: Throwable3D in active_throwables:
		pull_force_based_on_throwable_mode(body, delta)


func push_wave():
	if push_wave_ability:
		var wave = PushWaveArea.new(self, Camera3DWizard.forward_direction(get_viewport().get_camera_3d()))
		add_child(wave)
		wave.activate()
...
func pull_body(body: Throwable3D):
	...
	body.pull(get_random_free_slot())
	active_throwables.append(body)
...
func throw_body(body: Throwable3D):
	...
	body.throw(impulse)
	active_throwables.erase(body)
	
	if throwable_interactor:
		throwable_interactor.enabled = true
...
func get_random_free_slot() -> Marker3D:
	...
	return available_slots.pick_random()


func get_near_throwables() -> Array[Throwable3D]:
	if throwable_area_detector:
		var bodies := throwable_area_detector.get_overlapping_bodies().filter(func(body): return body is Throwable3D)
		bodies.sort_custom(func(a: Throwable3D, b: Throwable3D): return NodeWizard.global_distance_to_v3(a, actor) <= NodeWizard.global_distance_to_v3(b, actor))	
		return bodies
...
func _prepare_throwable_interactor(distance: float = throwable_interactor_distance):
	if throwable_interactor and distance >= 0.1:
		...
		throwable_interactor.collision_layer = 0
		throwable_interactor.collision_mask = 16 ## Throwables layer
...
func _prepare_throwable_area_detector():
	if throwable_area_detector:
		...
		throwable_area_detector.collision_mask = 16 ## Throwables layer
