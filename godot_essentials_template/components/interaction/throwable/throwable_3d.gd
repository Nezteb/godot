...
var original_collision_mask := 1 | 2 | 4 | 128 ## Interact with world, player, enemies and shards
...
func _ready():
	...
	active_material = NodeWizard.first_node_of_class(self, MeshInstance3D.new()).get_active_material(0)
...
func apply_transparency():
	...
	if active_material:
		...
		active_material.albedo_color = transparency_on_pull
...
func recover_transparency():
	...
	if active_material:
		active_material.albedo_color = original_transparency
